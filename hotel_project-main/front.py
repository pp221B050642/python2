...
def click_check_out_button():  #checking out function
    ...
    sigh_in_button=tk.Button(checkout,text="check out", bg="white", font=("AC Boucle", 30)).place(x=50, y=210)
    check_button=tk.Button(checkout,text="check", bg="white", font=("AC Boucle", 30), command=check_entry).place(x=250, y=210)
...
def click_reservation_button(): #reservation button
    ...
    name_entry=tk.Entry(reservation)
    name_entry.place(x=50, y=100, width=350, height=40)
    make_res=tk.Label(reservation, text="Please, register first", bg="white", font=("AC Boucle", 20)).place(x=50, y=60)
    password_entry=tk.Entry(reservation)
    password_entry.config(show="*")
    password_entry.place(x=50, y=150, width=350, height=40)
    register_button=tk.Button(reservation,text="registration",  bg="white", font=("AC Boucle", 30)).place(x=50, y=210)
...
def click_sighin_button(): #sigh in button
    ...
    name_entry.place(x=50, y=100, width=350, height=40)
    sigh_in_button=tk.Button(sigh_in,text="sigh_in", bg="white", font=("AC Boucle", 30)).place(x=50, y=210)
